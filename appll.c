...
void bisearch()
{
    ...
    int lower = 0, upper = -1, target, c=0;
...
printf("\nEnter the element to be found\n");
    ...
    while (lower <= upper) {
        ...
        // Check if the mid node contains the target value
        if (current->num == target) {
            printf("%d is found at %d",target,mid);
            c=1;
            break;
        }
        // If the target value is less than the mid node value, search the left half
        else if (current->num > target) {
            upper = mid - 1;
        }
        // If the target value is greater than the mid node value, search the right half
        else {
            lower = mid + 1;
        }
    }
